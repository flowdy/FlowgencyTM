...
my $ts1 = FTM::Time::Point->parse_ts($$cur_ts_string_ref);
isa_ok($ts1, 'FTM::Time::Point', "created object for timestamp $$cur_ts_string_ref");
...
my $ts2 = FTM::Time::Point->parse_ts($$cur_ts_string_ref);
...
my $ts3 = FTM::Time::Point->parse_ts($$cur_ts_string_ref);
...
my $ts4 = FTM::Time::Point->parse_ts($$cur_ts_string_ref)->fill_in_assumptions;
is("$ts4", "$ts3", 'leading zero omitted');
$cur_ts_string_ref = \(my $ts_test_string5 = '4 19:15');
my $ts5 = FTM::Time::Point->parse_ts($$cur_ts_string_ref)->fill_in_assumptions;
...
my $ts9 = FTM::Time::Point->parse_ts($$cur_ts_string_ref)->fill_in_assumptions;
is("$ts9", "$ts8", 'german, leading zero omitted');
$cur_ts_string_ref = \(my $ts_test_string10 = '4. 19:15');
my $ts10 = FTM::Time::Point->parse_ts($$cur_ts_string_ref)->fill_in_assumptions;
...
$tsa = FTM::Time::Point->parse_ts("20.");
$tsb = FTM::Time::Point->parse_ts("6.1.2012");
is($tsa->fix_order($tsb), 1, "timestamps in right order");
is($tsa->month, 12, " ... leaping Jan to Dec");
is($tsa->year, 2011, " ... leaping 2012 to 2011");

$tsa = FTM::Time::Point->parse_ts('2012-07-15');
...
$tsb = FTM::Time::Point->parse_ts('2012-07-15 17:00:00');
ok( $tsa < $tsb && $tsb < $tsa, 'compare whole day and particular time of it');
$tsb = FTM::Time::Point->parse_ts('2012-07-14 23:59:59');
ok( $tsa > $tsb && $tsb < $tsa, ' ... different days');
$tsb = FTM::Time::Point->parse_ts('2012-07-16 00:00:01');
...
$tsb = FTM::Time::Point->parse_ts('2012-07-15');
ok( $tsa == $tsb, 'same day');
$tsb = FTM::Time::Point->parse_ts('2012-07-15 0');
...
FTM::Time::Point->now("2010-05-12");
my $tsc = FTM::Time::Point->parse_ts("1.");
is( "".$tsc->fill_in_assumptions, "2010-05-01",
    "Changing base date with FTM::Time::Point->now()"
);
sleep 1;
is( FTM::Time::Point->now->sec, 1, 'Auto-syncing of now' );

$tsc = FTM::Time::Point->now();
my $tsc2 = FTM::Time::Point->now();
cmp_ok( refaddr($tsc), '!=', refaddr($tsc2), "Calling class method now() from FTM::Time::Point externally, get copy of underlying instance");
